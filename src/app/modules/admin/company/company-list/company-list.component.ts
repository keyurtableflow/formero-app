...
export class CompanyListComponent {
  ...
  ngOnInit() {
    ...
    this.getCompanyList(this.pageSize, this.currentPage);
  ...
  performSearch(input: any) {
    this.getCompanyList(this.pageSize, this.currentPage, input);
  }

  getCompanyList(size: number = 0, current: number, search: string = '', sortColumn: string = 'created_at', sortOrder: number = 1) {
    this.pageSize = size;
    ...
    this._companyService.getAllCompany(url).subscribe((result: any) => {
  ...
  onPageChange(e: any) {
    this.getCompanyList(e.pageSize, e.pageIndex, this.searchInputControl.value, this.sortColumn, this.sortOrder)
  }
  ...
  redirectToAddCompany() {
    this._router.navigateByUrl('company/add-company')
  }

  editCompany(id) {
    this._router.navigateByUrl('company/edit-company/' + id)
  }

  sortData(sort: Sort) {
    this.sortColumn = sort.active;
    this.sortOrder = sort.direction == 'asc' ? 1 : -1;
    this.getCompanyList(this.pageSize, this.currentPage, this.searchInputControl.value, sort.active, sort.direction == 'asc' ? 1 : -1)
  }
  ...
  openConfirmationDialog(data: any): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result == 'confirmed') {
        this._companyService.deleteCompany(this.selectedProductId).subscribe((response: any) => {
          if (response.statusCode == 200) {
            this.getCompanyList(0, 0);
