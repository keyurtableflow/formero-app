...
@Component({
    selector: 'app-material-variable',
    standalone: true,
    ...
    templateUrl: './material-variable.component.html',
    styleUrls: ['./material-variable.component.scss']
})
export class MaterialVariableComponent {
    ...
    constructor(
        ...
        this._route.params.subscribe(params => {
            ...
            this.getMaterialName(this.materialId);
        });
    }
    ...
    getMasterVariableList() {
        this._variableService.getAllVariable('?skip_pagination=true&search=MaterialVariable&type=Constant').subscribe((result: any) => {
    ...
    getMaterialName(materialId: string) {
        this._materialService.getMaterial(materialId).subscribe((res) => {
    ...
    fetchVariableList(
        ...
        let url = `?limit=${size}&page=${current + 1}&orderby=${sortColumn}&sort=${sortOrder}&material_id=${this.materialId}`;
        if (search != '') {
            url += `&search=${search}`;
        }
        this._materialService.getAllMaterialVariables(url).subscribe(
            (result: any) => {
    ...
    saveVariable(variable) {
        ...
        let printerVariable: MaterialVariable = new MaterialVariable( variable.variablesData?._id, this.materialId, variable.value);
    ...
    createVariable(variable) {
        this._materialService.addMaterialVariable(variable).subscribe(
            (result: any) => {
    ...
    updateVariable(variable) {
        this._materialService.updateMaterialVariable(variable._id, variable).subscribe(
            (result: any) => {
                variable.isEditing = false;
                this.currentlyEditingVariable = null;
                this.getVariableList(
        ...
        dialogRef.afterClosed().subscribe((result) => {
            if (result == 'confirmed') {
                this._materialService
                    .deleteMaterialVariable(this.selectedVariableId)
                    .subscribe(
                        (response: any) => {
...
class MaterialVariable {
    ...
    constructor( variablesId: string, materialId: string, value: string, id?: string) {
