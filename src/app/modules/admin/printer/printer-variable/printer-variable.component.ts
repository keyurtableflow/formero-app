...
@Component({
    ...
    templateUrl: './printer-variable.component.html',
    styleUrls: ['./printer-variable.component.scss']
})
export class PrinterVariableComponent {
    ...
    constructor(
        ...
        this._route.params.subscribe(params => {
            ...
            this.getPrinterName(this.printerId);
          });
    }
    ...
    getMasterVariableList(){
        this._variableService.getAllVariable('?skip_pagination=true&search=PrinterVariable&type=Constant').subscribe((result: any) => {
    ...
    getPrinterName(printerId:string){
        this._printerService.getPrinterById(printerId).subscribe((res)=>{
            this.printer = res.data.result;
        })
    }
    ...
    fetchVariableList(
        ...
        let url = `?limit=${size}&page=${current + 1}&orderby=${sortColumn}&sort=${sortOrder}&printer_id=${this.printerId}`;
        if (search != '') {
            url += `&search=${search}`;
        }
        this._printerService.getAllPrinterVariables(url).subscribe(
            (result: any) => {
    ...
    saveVariable(variable) {
        ...
        let printerVariable:PrinterVariable = new PrinterVariable(variable.variablesData?._id,this.printerId,variable.value);
    ...
    createVariable(variable) {
        this._printerService.addPrinterVariable(variable).subscribe(
            (result: any) => {
    ...
    updateVariable(variable) {
        this._printerService.updatePrinterVariable(variable._id, variable).subscribe(
            (result: any) => {
                variable.isEditing = false;
                this.currentlyEditingVariable = null;
                this.getVariableList(
        ...
        dialogRef.afterClosed().subscribe((result) => {
            if (result == 'confirmed') {
                this._printerService
                    .deletePrinterVariable(this.selectedVariableId)
                    .subscribe(
                        (response: any) => {
...
class PrinterVariable{
    ...
    constructor (variablesId:string,printerId:string,value:string,id?:string){
