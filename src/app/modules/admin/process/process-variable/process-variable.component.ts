...
export class ProcessVariableComponent {
    ...
    constructor(
        ...
        this._route.params.subscribe(params => {
            ...
            this.getProcessName(this.processId);
        });
    }
    ...
    getMasterVariableList() {
        this._variableService.getAllVariable('?skip_pagination=true&search=ProcessVariable&type=Constant').subscribe((result: any) => {
    ...
    getProcessName(processId: string) {
        this._processService.getProcessById(processId).subscribe((res) => {
    ...
    fetchVariableList(
        ...
        let url = `?limit=${size}&page=${current + 1}&orderby=${sortColumn}&sort=${sortOrder}&process_id=${this.processId}`;
        if (search != '') {
            url += `&search=${search}`;
        }
        this._processService.getAllProcessVariables(url).subscribe(
            (result: any) => {
    ...
    saveVariable(variable) {
        ...
        let printerVariable: ProcessVariable = new ProcessVariable(variable.variablesData?._id, this.processId, variable.value);
    ...
    createVariable(variable) {
        this._processService.addProcessVariable(variable).subscribe(
            (result: any) => {
    ...
    updateVariable(variable) {
        this._processService.updateProcessVariable(variable._id, variable).subscribe(
            (result: any) => {
                variable.isEditing = false;
                this.currentlyEditingVariable = null;
                this.getVariableList(
        ...
        dialogRef.afterClosed().subscribe((result) => {
            if (result == 'confirmed') {
                this._processService
                    .deleteProcessVariable(this.selectedVariableId)
                    .subscribe(
                        (response: any) => {
...
class ProcessVariable {
    ...
    constructor( variablesId: string, processId: string, value: string, id?: string) {
